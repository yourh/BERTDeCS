...
def get_data_cnf(data_cnf, data):
    data_cnf_ = {**data_cnf['template'], **data_cnf.get(data, {})}
    return {k_: v_.format(data=data) if isinstance(v_, str) else v_
            for k_, v_ in data_cnf_.items()}
...
@click.option('--labels', type=click.STRING, default='labels')
@click.option('--train', 'train_flag', is_flag=True)
@click.option('--train-name', type=click.STRING, default='train')
@click.option('--valid-name', type=click.STRING, default=None)
@click.option('--eval', 'eval_flag', type=click.STRING, default=None)
...
def main(data_cnf, model_cnf, model_id, train_flag, eval_flag, num_epochs, model_batch_size, device_batch_size,
         model_path, result_path, log_path, labels, train_name, valid_name, device, enable_amp, enable_dist,
         restore, pretrained, strict_load):
    ...
    data_name = get_data_cnf(data_cnf, train_name).get('name', data_cnf['data'])
    ...
    mlb = get_mlb(Path(data_cnf[labels]) if labels in data_cnf else None)
    ...
    if train_flag:
        ...
        train_loader = get_dataloader(get_data_cnf(data_cnf, train_name), features, mlb, device_batch_size,
                                      is_training=True)
        if valid_name:
            valid_loader = get_dataloader(get_data_cnf(data_cnf, valid_name), features, mlb, device_batch_size)
        else:
            valid_loader = None
    ...
    if eval_flag:
        for test_name in get_option_list(eval_flag):
            test_cnf = get_data_cnf(data_cnf, test_name)
            logger.info(f'Loading Test Set {test_name}')
            top_k = test_cnf.get('top_k', model_cnf.get('eval', {}).get('top_k', 100))
            test_loader = get_dataloader(test_cnf, features, mlb, device_batch_size)
