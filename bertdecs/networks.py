...
class BaseNet(nn.Module):
    ...
    def __init__(self, *, encoder_options=(), labels_list=None, log_indent=''):
        ...
        self.encoder = BERTEncoder(**dict(encoder_options))
...
class MLANet(BaseNet, network_name='MLANet'):
    ...
    def __init__(self, *, att_options=(), classifier_options=(), loss_options=(), **kwargs):
        ...
        self.attention = MLAttention(self.num_labels, self.hidden_size, log_indent=self.log_indent, **dict(att_options))
        self.classifier = self.get_classifier(**dict(classifier_options))
...
class CLNet(BaseNet, network_name='CLNet'):
    ...
    def __init__(self, *, ph_options=(), loss_options=(), **kwargs):
        super().__init__(**kwargs)
        self.projection_head = MLLinear(input_size=self.hidden_size, log_indent=self.log_indent, **dict(ph_options))
